...
type MountPath = { mountPath: string; };
...
export function renderServer<P>(
    ...
    title: string,
    type: React.SFC<P>,
    props: React.Attributes & MountPath & P): void;
export function renderServer<P,
    ...
    props: React.ClassAttributes<T> & MountPath & P): void;
export function renderServer(resp: Response, entryPointName: string, title: string,
    ...
    const stylesheetLinks = externalStylesheets
        .map(x => `<link rel="stylesheet" href="${props.mountPath}${x}">`)
        .join('\n');
    const jsAssets = entryPoint
        .filter(x => x.endsWith('.js'))
        .map(x => `<script type="text/javascript" src="${props.mountPath}${x}"></script>`)
        .join('\n');
