...
export async function forceCherryPick(workShell: Shell, sha: string): Promise<void> {
    let cmd: string[] = [
        ...
        // Disable rename detection -- rename/add, rename/rename, rename/delete conflicts
        // are a pain to resolve since they just leave the `ours` file untouched in the index
        // and the `theirs` file as an untracked file with a strange filename.
        '-X', 'no-renames',
