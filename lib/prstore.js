...
function getVersionCommits(shell, prNumber, version) {
  return gitLog(shell, ['--reverse', `pr/${prNumber}/${version}/base..pr/${prNumber}/${version}/head`]);
}
...
function* makeVersion(workShell, prNumber, version, base, head) {
  ...
  if (prevVersion && prevVersion.head && prevVersion.base) {
    ...
    yield workShell.checkCall('git', ['cherry-pick', '-X', 'theirs', prevVersion.base + '..' + prevVersion.head]);
