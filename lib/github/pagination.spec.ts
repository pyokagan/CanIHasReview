...
export class ForEachPageTest {
    ...
    async 'goes to the next page if present'(): Promise<void> {
        const expectedReply1 = { message: 'Hello World' };
        const expectedReply2 = { message: 'Goodbye World!' };
        this.baseNock
            .get('/a')
        ...
        let i = 0;
        await forEachPage(fetch, `${baseUrl}a`, {}, body => {
            switch (i++) {
            case 0:
                assert.deepStrictEqual(body, expectedReply1);
                break;
            case 1:
                assert.deepStrictEqual(body, expectedReply2);
                break;
            default:
    ...
    async 'resolves promises returned by the callback'(): Promise<void> {
        ...
        await forEachPage(fetch, `${baseUrl}a`, {}, async body => {
            switch (i++) {
            case 0:
    ...
    async 'throws error if next page url is not github'(): Promise<void> {
        ...
        try {
            ...
            assert.strictEqual(e.message, 'bad next page url: https://badapi.github.com/b');
            return;
        }
